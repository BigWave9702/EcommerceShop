...
type ProductFormMode = "create" | "edit" | "event";

const MODE_COPY: Record<
  ProductFormMode,
  { heading: string; breadcrumb: string; submitLabel: string; savingLabel: string }
> = {
  create: {
...
const ProductForm = ({
    ...
    defaultValues: initialData
      ? {
          ...
          tags: Array.isArray(initialData.tags)
            ...
            : initialData.tags,
          cash_on_delivery: initialData.cashOnDelivery,
          starting_date: toDateInputValue(initialData.starting_date),
          ending_date: toDateInputValue(initialData.ending_date),
        }
      : undefined,
  });

  const hasEventDates = Boolean(
    initialData?.starting_date && initialData?.ending_date
  );
  const showEventFields = isEvent || (isEdit && hasEventDates);
  ...
  const [images, setImages]=useState<(uploadedImage|null)[]>(() => {
    if (initialData?.images?.length) {
      const existing = initialData.images.map((img: any) => ({
        fileId: img.file_id,
        file_url: img.url,
      }));
      return existing.length < 8 ? [...existing, null] : existing;
    }
    return [null];
  });
  ...
  const onSubmit=async (data: any) => {
    try {
      ...
      toast.error(error?.response?.data?.message || "Error on Submit");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
            ...
            <div className="w-2/4">
              ...
              <div className="mt-2">
                <CustomProperties
                  control={control}
                  errors={errors}
                  initialValue={initialData?.custom_properties}
                />
              </div>
              ...
              <div className="mt-2">
                ...
                <select
                  ...
                  defaultValue={initialData?.cashOnDelivery || "yes"}
      ...
      <div className="mt-6 flex justify-end gap-3">
        <button
          ...
          disabled={loading}
        >
          {loading ? copy.savingLabel : copy.submitLabel}
        </button>
      </div>
    </form>
  );
